...
function updateNearestArea() {
  ...
  locationArea.textContent = `你们现在靠近 ${nearest.stop.title} · ${formatDistance(nearest.distance)}`;
}
...
function requestLocation() {
  if (!("geolocation" in navigator)) {
    locationArea.textContent = "这个浏览器不支持实时定位";
    return;
  }

  locationArea.textContent = "正在定位...";
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    () => {
      locationArea.textContent = "定位未开启，点击按钮授权后可显示当前位置";
    },
    {
...
heroLocateButton.addEventListener("click", () => {
  requestLocation();
